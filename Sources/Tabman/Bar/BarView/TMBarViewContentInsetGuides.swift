...
internal final class TMBarViewContentInsetGuides: TMBarLayoutInsetGuides {
    ...
    init<Layout, Button, Indicator>(for barView: TMBarView<Layout, Button, Indicator>) {
        ...
        if #available(iOS 11, *) {
            leadingInset.leadingAnchor.constraint(equalTo: barView.crs_safeAreaLeadingAnchor).isActive = true
        } else {
        ...
        if #available(iOS 11, *) {
            trailingInset.trailingAnchor.constraint(equalTo: barView.crs_safeAreaTrailingAnchor).isActive = true
        } else {
